...
%L3 Marvin's data
indsSummaryFile = find(i_FullField_OFF_2s_OFF_2s.*i_UASGCaMP6f_L3MH56Gal4_cross_to_w_mars.*~i_moving);

%L2 Marvins's data
% indsSummaryFile = find(i_FullField_OFF_2s_OFF_2s.*i_UASGCaMP6F_L221DhhGal4_cross_to_w_mars.*~i_moving);
...
% Matrix
data = aggregate_Adaptingcontrast(neurData,OFF_stimulus);
data.OFF_stimulus = OFF_stimulus;
%This function sorts the negative and positive correlated ROIs 
CorrelationA = crosscorrelation_negative(data); 
    
negativecorr = CorrelationA.negresponse;
    
negativecorr = logical(negativecorr);
Sample_size = length(unique(data.flyID));
    
DataSave = ('MH56_Control');
%% Test plots to check data
%The data for all steps is ploted together in one plot with several
%subplots and independently for all the steps.
N_ROIs = size(data.rats,1);
N_EPOCHs = size(data.rats,2);
for cc = 1:N_EPOCHs
  rats(:,cc,:) = data.rats(negativecorr,cc,:);
...
cur_t5 = linspace(0,1,size(data.rats,2))';
cur_t = linspace(0,1,size(data.rats,3))';
...
for kk=1:num_steps-1
    ...
    traces = squeeze(data.rats(negativecorr,kk,:)); %because mean_cat_full works only with 2 dimensions %taking all ROIs, the epoch we are one, full lenth (200)
    [x,m,e] = mean_cat_full(traces,1,data.flyID(negativecorr)); %m=mean; e=std
...
%% B step (Contrast step)

num_steps = size(mean_val,1);
% [x,m,e] = mean_cat_full(epochRats,2,cur_ROIs); %m=m
% cat = category (flyID)
% epochRat = max_data
...
%   Contrast_stepforAnova(:,kk) = ContrastStep; 
...
for kk=1:num_steps
    ...
    baseline_Contrast = crossing_rats(:,kk,(120:150)); %8-10s for 2*10s step
    baseline_Contrast = squeeze(baseline_Contrast);
    %defining the time where we look for the response
    response_Contrast = crossing_rats(:,kk,(210:230)); %I think this is wrong?!? This is the grey period already!
...
for kk=1:num_steps
    ...
    %ks: defining the time where we look for the baseline
    baseline_BStep = crossing_rats(:,kk,(170:190)); %8-10s for 2*10s step
    baseline_BStep = squeeze(baseline_BStep);
    %ks: defining the time where we look for the response
    response_BStep = crossing_rats(:,kk,(180:210)); %10-12s for 2*10s step
...
%     case 0.6
%         x1 = x1 / 4;
%         real_luminance = real_luminance/4;
%         Difference_luminance = Difference_luminance/4;
%     case 1
%         x1 = x1 / 10;
%     otherwise
%         x1 = x1;
...
% Weighted fitt; not used at the moment
% clear Regression1
% Regression1(1,:) = real_luminance;
% Regression1(2,:) = mean_calciumStep;
% Regression1(3,:) = e1;
% Regression1 = Regression1';
% hold on
% Fitt1 = weightedfit (Regression1);

%This calculates a regression line and a confidence interval for that
%regression line. It is so far not possible to plot a weighted regression
%line plus the convidence interval to that regression line.


%Not weighted fitt with confidence intervals for the first step
...
%     case 0.6
%         x2 = x2 / 4;
%         real_luminance1 = real_luminance1/4;
%         Difference_luminance1 = Difference_luminance1/4; 
%     case 1
%         x2 = x2 / 10;
%     otherwise
%         x2 = x2;
...
hold on
% Luminanceplot(5) = scatter(x2,y2,80, 'filled','MarkerFaceColor',[0.1 0.6 0.8] ,'DisplayName','Response to B step'); %circle
...
% clear Regression2
% Regression2(1,:) = real_luminance1;
% Regression2(2,:) = mean_ContrastStep;
% Regression2(3,:) = e2;
% Regression2 = Regression2';
% hold on
% Fitt2 = weightedfit (Regression2);


%Not weighted fitt with confidence intervals for the second step

%errorbar(x2,y2,e2,'none','b','DisplayName','Standart error of the mean');
...
% fittedX2 = sort(fittedX2,'descend');
...
% %% Calculate Weber contrast from response differences
% %
% 
% figure('Units', 'pixels', 'Position',[100 100 500 375]);
% real_diff_contrast = real_luminance1;
% mean_weber_contrast = mean_ContrastStep - mean_calciumStep;
%  x1 = real_diff_contrast; %real_diff_contrast;
% y1 = mean_weber_contrast;
% 
% Condiff(1) = scatter(x1,y1,75,'k','filled','Displayname','dF/F(B Step)-dF/F(A Step)'); 
% xlabel('Contrast difference A - B (%)');
% ylabel('Difference (dF/F)');
% 
% 
% [coeffs1,S1] = polyfit(x1, y1, 1);
% % Get fitted values
...
% % Plot the fitted line
% hold on;
% Condiff(2) = plot(fittedX1, fittedY1, 'k--', 'LineWidth', 2,'DisplayName','Linear regression');
% Error_Fit1 = polyval(coeffs1,x1);
% T1 = table(x1,y1,Error_Fit1,y1-Error_Fit1,'VariableNames',{'X','Y','Fit','FitError'});
% % Calculate the confidence values and plot the confidence lines 
% % The error is not yet included
% [Y1,DELTA1] = polyconf(coeffs1,fittedX1,S1,'alpha',0.05);
% % fittedX1 = sort(fittedX1,'descend');
% Condiff(3) = plot(fittedX1,Y1+DELTA1,'k--','DisplayName','95% confidence interval');
% plot(fittedX1,Y1-DELTA1,'k--');
% 
% legend (Condiff(1:3),'location','best');
% ax=gca;
% ax.FontSize = 20;
% if DataSave == 'L3 Control'
%     ylim([-10 40])
% elseif DataSave == 'L2 Control'
% ylim ([-80 20]);
% end
% xlim ([10 80]);
% grid on
...
%   'LineWidth'   , 1         );
% if autosave ==1
% print([num2str(Savelocation),'\Difference_Response_Contrast',num2str(DataSave),num2str(NDFilter)],'-depsc')
% end
% hold off;
%% 
y1 = mean_BStep(1:6); %ks
x1 =     real_luminance; %Difference_luminance;  %real_luminance;
% switch ndFilter
%     case 0.6
%         x1 = x1 / 4;
...
grid on;
hold on;
Luminanceplot(1) = scatter(x1,y1,75,'filled','r','DisplayName','Response to A step');
Luminanceplot(2) = errorbar(x1,y1,e1,'r','Linestyle','none','DisplayName','SEM');
% Weighted fitt; not used at the moment
% clear Regression1
% Regression1(1,:) = real_luminance;
% Regression1(2,:) = mean_calciumStep;
...
[Y1,DELTA1] = polyconf(coeffs1,fittedX1,S1,'alpha',0.05);
% fittedX1 = sort(fittedX1,'descend');
Luminanceplot(4) = plot(fittedX1,Y1+DELTA1,'r--','DisplayName','95% confidence interval');
plot(fittedX1,Y1-DELTA1,'r--');
...
% 
% hold on
% Luminanceplot(5) = scatter(x2,y2,75,'filled','b','DisplayName','Response to B step');
% hold on
% Luminanceplot(6) =  errorbar(x2,y2,e2,'b','Marker','none','Linestyle', 'none','MarkerFaceColor','b','DisplayName','SEM');
% hold on
% 
% 
% 
...
% [Y2,DELTA2] = polyconf(coeffs2,fittedX2,S2,'alpha',0.05);
% % fittedX2 = sort(fittedX2,'descend');
% Luminanceplot(8) = plot(fittedX2,Y2+DELTA2,'b--','DisplayName','95 % confidence interval');
% plot(fittedX2,Y2-DELTA2,'b--', 'DisplayName','none');
% 
% 
% 
% hold on 
...
%% Perform Anova for the contrast step
% ktest = kstest(Contrast_stepforAnova(:,(1:6)));
%  if ktest == 0
% [Anova_Contrast,tbl,Anovafigure] = anova1(Contrast_stepforAnova(:,(1:6)));
% hold on
% ax=gca;
% ax.FontSize = 20;
% ylim([-0.4 1.8]);
% grid on
% title (['Boxplot for 25% contrast step responses',num2str(DataSave)]);
% xlabel('Steps');
% ylabel('dF/F calcium signal') 
% set(gca, ...
%   'Box'         , 'off'     , ...
...
% 
% if autosave ==1
% print([num2str(Savelocation),Dataslash,'Evaluating_Contrast_step_Anova',num2str(DataSave),num2str(NDFilter)],'-depsc');
% save([num2str(Savelocation),Dataslash,'Evaluating_Contrast_step_Anova',num2str(DataSave),num2str(NDFilter)],'tbl');
% end
%  
%  else
%      fprintf 'Data does not follow standard distribution';
  end
%% Save data for further analysis
% Anova between L2 and L3 responses and responses to same and differen 
% contrast steps
% 
% save([Savelocation(1:end-7),Dataslash,'Anova_Data',Dataslash,num2str(DataSave),NDFilter,'_mean_CalciumStep'],'mean_calciumStep','err_calciumStep','Sample_size');
% save([Savelocation(1:end-7),Dataslash,'Anova_Data',Dataslash,num2str(DataSave),NDFilter,'_mean_ContrastStep'],'mean_ContrastStep','err_ContrastStep');
% 
% save([Savelocation(1:end-7),Dataslash,'Anova_Data',Dataslash,num2str(DataSave),NDFilter,'_ContrastStep'],'Contrast_stepforAnova');
% save([Savelocation(1:end-7),Dataslash,'Anova_Data',Dataslash,num2str(DataSave),NDFilter,'_CalciumStep'],'Calcium_stepforAnova');
% 
% 
